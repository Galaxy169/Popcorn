...
export default function StarRatings({ maxRating = 5, color = "#fcc419", size = 48, className = "", messages = [], defaultRating = 0, onSetRating }) {
    ...
    <div style={containerStyle} className={className}>
      <div style={starContainerStyle}>
        {Array.from({ length: maxRating }, (_, i) => (
          <Star onRate={() => handleRating(rating === i + 1 ? 0 : i + 1)} full={tempRating ? tempRating >= i + 1 : rating >= i + 1} key={i} onHoverIn={() => setTempRating(i + 1)} onHoverOut={() => setTempRating(0)} color={color} size={size} />
        ))}
      </div>
